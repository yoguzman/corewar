...
void	ld(t_corewar *vm, t_proc *lol, t_instr *instr)
{
	...
	(void)instr;
	if (lol->inv.val_arg[0] == DIR_SIZE)
	{
		lol->reg[lol->inv.param[1] - 1] = lol->inv.param[0];
	}
	else
	{
		offset = (lol->inv.save_pc + (lol->inv.param[0] % IDX_MOD));
		i = -1;
...
void	ldi(t_corewar *vm, t_proc *lol, t_instr *instr)
{
	...
	if (!vm->visual)
		ft_printf("P\t%u | ldi %d %d r%d\n", lol->pid,
				(lol->inv.val_arg[0] == T_REG
				? lol->reg[lol->inv.param[0] - 1] :
				lol->inv.param[0]),
				(lol->inv.val_arg[1] == T_REG ?
				 lol->reg[lol->inv.param[1] - 1] : lol->inv.param[1]),
				lol->inv.param[2]);
}
...
void	st(t_corewar *vm, t_proc *lol, t_instr *instr)
{
	...
	(void)instr;
	if (lol->inv.val_arg[1] == T_REG)
		lol->reg[lol->inv.param[1] - 1] = lol->reg[lol->inv.param[0] - 1];
	else
	{
		i = -1;
		offset = (lol->inv.save_pc + (lol->inv.param[1] % IDX_MOD));
		while (++i < 4)
		{
			vm->arena[((uint32_t)offset + i) % MEM_SIZE] =
				(lol->reg[lol->inv.param[0] - 1] >> ((3 - i) << 3)) & 0xFF;
		}
		if (vm->visual == 1)
			print_4b_in_arena((uint32_t)offset % MEM_SIZE, vm->arena, lol, i);
	}
	lol->carry = lol->reg[lol->inv.param[0] - 1] == 0;
	if (!vm->visual)
...
void	sti(t_corewar *vm, t_proc *lol, t_instr *instr)
{
	...
	to_jump = (lol->inv.save_pc + (to_jump % IDX_MOD));
	i = -1;
	while (++i < 4)
	{
	...
	if (!vm->visual)
		ft_printf("P\t%u | sti r%d %d %d to_jump = %d write = %u\n", lol->pid,
				...
				? lol->reg[lol->inv.param[1] - 1] :
				lol->inv.param[1]),
				(lol->inv.val_arg[2] == T_REG ?
				lol->reg[lol->inv.param[2] - 1] : lol->inv.param[2]), (to_jump + i) % MEM_SIZE, lol->reg[lol->inv.param[0] - 1]);
}
